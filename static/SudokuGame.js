import {groupBy, uniq, uniqBy, SUDOKU_NUMBERS} from "./utility.js";

export default class SudokuGame {
  static uniquePaths(paths) {
  ...
  static getPathBetweenCell(c1, c2) {
    const {x: x1, y: y1} = c1;
    const {x: x2, y: y2} = c2;
  ...
  static getCellPosition(c, width, height) {
  ...
  static positionedCells(sudoku, width, height) {
    return sudoku.map((c) => this.getCellPosition(c, width, height));
  }

  static correct(sudoku) {
    if (!sudoku) {
      return false;
    }
  ...
  static sameNumber(cell, sudoku) {
    return sudoku.filter((c) => c.number === cell.number);
  }

  static sameSquareColumnRow(cell, sudoku) {
  ...
  static conflictingFields(sudoku) {
    const sudokuWithIndex = sudoku.map((c, i) => ({
      ...c,
      index: i,
    }));
  ...
  static getPathsFromConflicting(conflictingCell, sudoku) {
